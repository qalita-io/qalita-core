...
def chunked_parquet_files(temp_parquet_dir):
    ...
    for i in range(3):
        df = pl.DataFrame({
            ...
            "value": [f"value_{j}" for j in range(i * 10000, (i + 1) * 10000)],
            "amount": [float(j) * 1.5 for j in range(i * 10000, (i + 1) * 10000)],
        })
        path = os.path.join(temp_parquet_dir, f"test_data_part_{i + 1}.parquet")
        df.write_parquet(path)
        paths.append(path)
    return paths
...
class TestDataSourceOpenerPolars:
    ...
    def test_file_source_uses_polars_for_large_csv(self, temp_parquet_dir):
        ...
        paths = source.get_data(pack_config={
...
class TestPackPolarsIntegration:
    ...
    def test_scan_data_returns_lazyframe(self, temp_parquet_dir, monkeypatch):
        ...
        source_conf = {"name": "test", "type": "file", "config": {"path": parquet_path}}
        pack_conf = {"job": {}}
