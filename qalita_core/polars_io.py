...
def stream_to_parquet_chunks(
    ...
    *,
    ...
    for i in range(num_chunks):
        ...
        output_path = os.path.join(output_dir, f"{base_name}_part_{i + 1}.parquet")
        
        try:
            chunk_lf.sink_parquet(output_path, compression=compression)
        except Exception:
            # Fallback
            chunk_lf.collect().write_parquet(output_path, compression=compression)
...
def collect_streaming(
    ...
    *,
    ...
    try:
        ...
        logger.warning("Streaming collection failed, falling back to standard collect")
        return lf.collect()
...
def estimate_memory_usage(lf: "pl.LazyFrame") -> Dict[str, Any]:
    ...
    for dtype in schema.values():
        if dtype in (pl.Int8, pl.UInt8, pl.Boolean):
            ...
            bytes_per_row += 4
        elif dtype in (pl.Int64, pl.UInt64, pl.Float64, pl.Datetime, pl.Duration):
            bytes_per_row += 8
